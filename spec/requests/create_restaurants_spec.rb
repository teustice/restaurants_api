...
describe "create endpoint", :type => :request do
  ...
  before do
    post '/restaurants', params: {name: "test_name", category: "test_category", price_range: "test_price_range", address: "test_address"}
  end
  ...
  it 'returns the restaurant price_range' do
    expect(JSON.parse(response.body)['data']['attributes']['price-range']).to eq('test_price_range')
  end
